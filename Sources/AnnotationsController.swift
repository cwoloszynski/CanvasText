...
final class AnnotationsController {
	...
	func insert(block: BlockNode, index: Int) {
		guard enabled, let block = block as? Annotatable,
			let annotation = annotationForBlock(block: block)
		else {
	...
	func update(block: BlockNode, index: Int) {
		guard enabled && index < annotations.count,
			let block = block as? Annotatable,
	...
	func rectForAnnotation(annotation: Annotation) -> CGRect {
		...
		case .ExpandedLeadingGutter:
			guard let rects = rectsForPresentationRange(presentationRange: presentationRange),
